...
test('Times test', async ({ page, extensionId }) => {
  ...
  await page.locator("_react=TimesForm >> select").selectOption({label:"day"})
  await page.locator("_react=TimesForm >> _react=[aria-label = 'Add time']").click()

  // remove time
  await page.locator("_react=TimesTable >> tr:has-text('6 month') >> _react=[aria-label = 'Delete time']").click()

  // check
  await page.goto('https://www.google.com/search?q=hoge');
  for (const time of ["Any", "1 day", "3 day", "1 week", "1 month", "1 year", "3 year"]) {
  ...
  // check
  await page.goto('https://www.google.com/search?q=hoge');
  for (const time of ["Any", "1 day", "1 month", "6 month", "1 week", "3 year"]) {
    await expect(page.locator('#choomameRoot')).toHaveText(new RegExp(time));
  }
});
